-- Protect consolidated card invoices when rolling back a later import.
--
-- Bug: upsertCardStatementCycle overwrote import_batch_id on conflict
-- (account_id, closing_date). Rolling back the newer batch then deleted
-- older invoices via:
--   delete from card_statement_cycles where import_batch_id = p_batch_id
--
-- Defense in depth (besides app-side upsert provenance preservation):
-- unlink cycles that still have remaining activity or a trusted issuer
-- amount_due; only delete cycles that are empty leftovers of this batch.
...
begin
  ...
  -- Unlink consolidated cycles instead of deleting them.
  with unlinked as (
  ...
  -- Delete only empty leftovers still owned by this batch.
  with deleted as (
...
comment on function public.rollback_import_batch(uuid) is
  'Atomically undoes one import batch. Preserves category memory. Unlinks consolidated card_statement_cycles (issuer amount_due or remaining activity) instead of deleting them; deletes only empty leftovers.';
