-- Extend cleanup_finance_data to clear import history when wiping
-- transactions/accounts, including credit-card import batches/rows.
-- Account selection remains ownership/family based (no type exclusion):
-- checking, savings, cash, investment, and credit_card are all in scope.
...
begin
  ...
  -- All owned/shared accounts in scope (bank + credit_card; real + forecast).
  ...
  if v_do_transactions or v_do_accounts then
    ...
    -- Clear import history for scoped accounts (bank + credit card).
    -- import_batch_rows cascade from import_batches.
  ...
  if v_do_transactions and not v_do_accounts then
    if cardinality(v_account_ids) > 0 then
      ...
      -- Explicit balance reset so bank + credit card stay consistent after wipe.
      with updated as (
...
comment on function public.cleanup_finance_data(text[], uuid) is
  'Selectively deletes the caller''s financial data (bank + credit card accounts, transactions, import history) and active-family shared data when admin. Never deletes auth, profile, family graph, or categories.';
