-- Prepare invoice payments for manual vs imported origin and future reconciliation.
-- Both legs of an invoice payment (source expense + card income) should carry these fields.

alter table public.transactions
  add column if not exists invoice_payment_origin text
    check (
      invoice_payment_origin is null
      or invoice_payment_origin in ('manual', 'imported')
    );

alter table public.transactions
  add column if not exists reconciled_with_transaction_id uuid
    references public.transactions (id) on delete set null;

comment on column public.transactions.invoice_payment_origin is
  'Origin of a credit-card invoice payment leg: manual UI vs CSV import. Null for non-payment transactions.';

comment on column public.transactions.reconciled_with_transaction_id is
  'Future link between equivalent manual and imported invoice payment legs. When set, settlement should not double-count.';

create index if not exists transactions_invoice_payment_origin_idx
  on public.transactions (invoice_payment_origin)
  where invoice_payment_origin is not null;

create index if not exists transactions_reconciled_with_transaction_id_idx
  on public.transactions (reconciled_with_transaction_id)
  where reconciled_with_transaction_id is not null;

-- Persist optional invoice_payment_origin from import commit payload.
...
begin
  ...
  if p_source not in ('nubank_checking', 'nubank_credit_card') then
  ...
  loop
    if exists (
      ...
      raise exception 'Row already imported: %', v_row ->> 'identity_key';
    end if;
