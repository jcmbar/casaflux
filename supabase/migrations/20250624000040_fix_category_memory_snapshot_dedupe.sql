-- Fix snapshot_category_classification_memory:
-- ON CONFLICT key is (owner_user_id, normalized_description, transaction_type, category_id)
-- but the source CTE previously grouped by raw description + family_id, so multiple
-- rows could collide on the same conflict target in one INSERT
-- ("ON CONFLICT DO UPDATE command cannot affect row a second time").
--
-- Prefer family_id via DISTINCT ON + boolean sort (Postgres has no max(uuid)).
...
begin
  ...
  -- Aggregate counts / last_seen per conflict key (no uuid aggregates).
  tallies as (
    select
      owner_user_id,
  ...
  -- One representative row per key: prefer non-null family_id, then newest seen_at.
  best as (
    select distinct on (
      owner_user_id,
      normalized_description,
