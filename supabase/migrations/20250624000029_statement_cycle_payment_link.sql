-- Link invoice payments to a credit-card statement cycle without a new table.
-- cycle id = closing date ISO (YYYY-MM-DD), matching StatementCycle.cycleId.

alter table public.transactions
  add column if not exists statement_cycle_id text;

comment on column public.transactions.statement_cycle_id is
  'Credit-card statement cycle id (closing date YYYY-MM-DD) for invoice payments. Null for other transactions and legacy payments.';

create index if not exists transactions_statement_cycle_id_idx
  on public.transactions (statement_cycle_id)
  where statement_cycle_id is not null;

-- Persist optional statement_cycle_id from import commit payload.
